  ...
  return (
    <View className="gap-1.5">
      <View className="flex-row flex-wrap gap-1.5">
        {report.series.map((s) => {
          ...
          return (
            <Pressable
              ...
              accessibilityLabel={`${on ? 'Hide' : 'Show'} ${s.name} on chart`}
              onPress={() => onToggle(s.playerId)}
              className={`flex-row items-center gap-1.5 rounded-full border px-2.5 py-1 active:opacity-80 ${
                on ? 'border-line bg-background/50' : 'border-line/50 bg-transparent'
              }`}
            >
              <View
              ...
              />
              <Text
                className={`max-w-[7rem] text-xs font-medium ${
                  on ? 'text-zinc-100' : 'text-muted-foreground line-through'
                }`}
                numberOfLines={1}
              >
                {s.name}
        ...
        {!allVisible ? (
          <Pressable onPress={onShowAll} hitSlop={6} className="px-2 py-1 active:opacity-70">
            <Text className="text-xs text-muted-foreground underline">Show all</Text>
          </Pressable>
        ) : null}
      </View>
      <Muted>Tap a name to show or hide their line.</Muted>
    </View>
  )
}
...
function BalanceOverTimeChart({
  ...
  const dataSet = visibleSeries.map((series) => {
  ...
  return (
    <View className="mt-2 gap-3 overflow-hidden">
      <PlayerSeriesKey
        report={report}
        hidden={hidden}
        onToggle={(playerId) =>
      ...
      {report.rows[0] ? (
        <Muted>
          ...
          {report.rows.length > 1 ? ` → ${formatDate(report.rows[report.rows.length - 1]!.at)}` : ''}
        </Muted>
      ) : null}
    </View>
  )
